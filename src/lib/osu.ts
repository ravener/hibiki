...
export const api = new osu.API(CLIENT_ID, CLIENT_SECRET, { verbose: 'all' });
...
export function formatGameMode(ruleset: osu.Ruleset | keyof typeof osu.Ruleset): string {
    switch (ruleset) {
        case 'osu':
        case osu.Ruleset.osu:
            return 'osu! Standard';

        case 'mania':
        case osu.Ruleset.mania:
            return 'osu!mania';

        case 'fruits':
        case osu.Ruleset.fruits:
            return 'osu!catch';

        case 'taiko':
        case osu.Ruleset.taiko:
            return 'osu!taiko';
    }
}
